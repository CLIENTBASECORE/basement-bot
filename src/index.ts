...
\x1b[0m
  \x1b[90m┌──────────────────────────────────────────────────┐\x1b[0m
  \x1b[90m│\x1b[0m \x1b[32m●\x1b[0m \x1b[1mBasement Cinema Network • Standalone Discord Bot\x1b[0m  \x1b[90m│\x1b[0m
...
async function ensurePublicCommandsUnlocked(c: Client) {
  try {
    ...
    if (everyone && !everyone.permissions.has(PermissionFlagsBits.UseApplicationCommands)) {
      ...
      console.log('[Basement Permissions] Enabled UseApplicationCommands on @everyone role.');
    }
    ...
    for (const [id, ch] of channels) {
      ...
      if (overwrite && overwrite.deny.has(PermissionFlagsBits.UseApplicationCommands)) {
        ...
        console.log(`[Basement Permissions] Cleared command deny on ${ch.name}`);
      }
    }
  } catch (err) {
    console.warn('[Basement Permissions] Auto-unlock public commands notice:', err);
  }
}

// Event: Ready
client.once(Events.ClientReady, async c => {
  console.log(`\x1b[32m[Basement Gateway] Logged in as ${c.user.tag}\x1b[0m`);

  // Presence / RPC: "Watching movies & shows on Basement"
  c.user.setPresence({
    status: 'online',
    activities: [
      {
        name: 'movies & shows on Basement',
        type: ActivityType.Watching,
        url: 'https://basementx.lol',
      },
    ],
  });

  console.log(`[Basement Gateway] Presence active: "Watching movies & shows on Basement"`);

  // Register all 16 commands to Discord Guild
...
process.on('unhandledRejection', reason => {
  console.error('[Basement Gateway] Handled rejection:', reason);
});
process.on('uncaughtException', error => {
  console.error('[Basement Gateway] Handled exception:', error);
});

client.on(Events.Error, error => {
  console.error('[Basement Client Error]', error);
});
...
client.on(Events.InteractionCreate, async (interaction: Interaction) => {
  try {
    ...
    console.error('[Basement Interaction Error]', error?.message || error);
...
client.on(Events.MessageCreate, async message => {
  ...
  // 2. Mention bot to talk with Basement AI (if not a prefix command)
  if (client.user && message.mentions.has(client.user) && !message.content.startsWith(db.getPrefix())) {
    try {
      ...
      console.warn('[Basement Chat Mention Error]', err);
    }
  }
});
...
if (CONFIG.DISCORD_TOKEN) {
  client.login(CONFIG.DISCORD_TOKEN).catch(err => {
    console.error('\x1b[31m[Basement Gateway] Failed to connect with DISCORD_TOKEN:\x1b[0m', err.message);
  });
} else {
  console.warn(`\x1b[33m[Basement Gateway] WARNING: No DISCORD_TOKEN found in environment variables!\x1b[0m`);
  console.warn(`\x1b[33m[Basement Gateway] On Render.com: Go to your service Dashboard -> Environment -> Add DISCORD_TOKEN, CLIENT_ID, and GUILD_ID.\x1b[0m`);
}
...
server.listen(port, '0.0.0.0', () => {
  console.log(`\x1b[36m[Basement Keep-Alive] HTTP server listening on 0.0.0.0:${port} for Render & UptimeRobot\x1b[0m`);
});
