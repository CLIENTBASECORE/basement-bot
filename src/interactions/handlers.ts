...
export async function handleButtonInteraction(interaction: ButtonInteraction): Promise<void> {
  ...
  if (customId.startsWith('wp_rsvp_')) {
    ...
    if (wp) {
      const updatedPayload = BasementEmbeds.watchParty(wp);
  ...
  if (customId.startsWith('req_')) {
    ...
    const payload = BasementEmbeds.requestTicket(updated);
  ...
  if (customId.startsWith('share_')) {
    ...
    await interaction.reply({
      content: `🎬 Stream **${item.title}** (${item.year}) on Basement: ${item.zenoxUrl}`,
    });
    return;
  }
