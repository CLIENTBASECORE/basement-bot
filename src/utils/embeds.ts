...
export class BasementEmbeds {
  /**
   * Base aesthetic embed with Basement styling and branding
   */
  public static base(title?: string, description?: string, color: number = BASEMENT_COLORS.emerald): EmbedBuilder {
    const embed = new EmbedBuilder()
      .setColor(color)
      .setTimestamp()
      .setFooter({
        text: `${BASEMENT_BRANDING.name} Network • ${BASEMENT_BRANDING.tagline}`,
  ...
  // ==========================================
  // ROLE PANELS (Mirroring Cinejoy / Basement Onboarding)
  // ==========================================
  ...
  public static createColorPanel(customRoles?: ColorRoleDef[]): { embeds: EmbedBuilder[]; components: ActionRowBuilder<ButtonBuilder>[] } {
    ...
    const embed = new EmbedBuilder()
      ...
      .setFooter({
        text: 'Basement Role Dispatcher • Select one color at a time',
        iconURL: BASEMENT_BRANDING.avatarUrl,
      });
  ...
  public static createPingPanel(): { embeds: EmbedBuilder[]; components: ActionRowBuilder<ButtonBuilder>[] } {
    ...
    const embed = new EmbedBuilder()
      ...
      .setColor(BASEMENT_COLORS.emerald)
      .setFooter({
        text: 'Basement Role Dispatcher • Click to toggle on/off',
        iconURL: BASEMENT_BRANDING.avatarUrl,
      });
  ...
  public static createShowPanel(): {
    ...
    const embed = new EmbedBuilder()
      ...
      .setColor(BASEMENT_COLORS.emerald)
      .setFooter({
        text: 'Basement TV Show Hub • Select from menu or buttons to unlock channels',
        iconURL: BASEMENT_BRANDING.avatarUrl,
      });
  ...
  public static mediaDetail(item: MediaItem): { embeds: EmbedBuilder[]; components: ActionRowBuilder<ButtonBuilder>[] } {
    ...
    const embed = new EmbedBuilder()
      ...
      .setFooter({
        text: `Basement Streaming Engine • Instant 4K Playback`,
        iconURL: BASEMENT_BRANDING.avatarUrl,
      });
  ...
  public static trendingList(items: MediaItem[]): { embeds: EmbedBuilder[]; components: ActionRowBuilder<ButtonBuilder>[] } {
    let desc = `Top trending movies and TV series streaming on **Basement** today (verified by live TMDB charts):\n\n`;
  ...
  public static requestTicket(req: ContentRequest): { embeds: EmbedBuilder[]; components: ActionRowBuilder<ButtonBuilder>[] } {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(`A community member requested a title to be added to the Basement library.`)
      .addFields(
    ...
    embed.setFooter({
      text: `Basement Request Queue • Ticket #${req.id}`,
      iconURL: BASEMENT_BRANDING.avatarUrl,
    });
  ...
  public static watchParty(wp: WatchParty): { embeds: EmbedBuilder[]; components: ActionRowBuilder<ButtonBuilder>[] } {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: '🌐 Direct Stream', value: `[Watch on Basement](${wp.streamUrl})`, inline: true }
      )
      .setImage('https://images.unsplash.com/photo-1518709268805-4e9042af9f23?w=1200')
      .setFooter({
        text: 'Basement Watch Party • Click RSVP to get pinged at showtime',
        iconURL: BASEMENT_BRANDING.avatarUrl,
      });
    ...
    const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
      ...
      new ButtonBuilder()
        ...
        .setURL(wp.streamUrl),
      new ButtonBuilder()
        .setLabel('🍿 Basement Watch Party Room')
        .setStyle(ButtonStyle.Link)
        .setURL(partyRoomUrl)
    );
  ...
  public static systemStatus(nodes: NodeStatus[]): { embeds: EmbedBuilder[]; components: ActionRowBuilder<ButtonBuilder>[] } {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        ...
        `Enjoy buffer-free streaming with dynamic subtitles at [**basementx.lol**](https://basementx.lol).`
      )
      .setFooter({
        text: 'Basement Cinema Network • Live Website Telemetry',
        iconURL: BASEMENT_BRANDING.avatarUrl,
      });
    ...
    const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder()
        ...
        .setStyle(ButtonStyle.Secondary),
      new ButtonBuilder()
        .setLabel('▶ Stream on basementx.lol')
        .setStyle(ButtonStyle.Link)
        .setURL('https://basementx.lol')
    );
  ...
  public static domainList(): { embeds: EmbedBuilder[]; components: ActionRowBuilder<ButtonBuilder>[] } {
    let desc = `Official working domains, anti-ISP proxies, and dedicated stream CDNs for **Basement**.\n*Bookmark these to avoid ISP DNS filters:*\n\n`;
    ...
    const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder()
        .setLabel('🚀 Visit Official (basementx.lol)')
        .setStyle(ButtonStyle.Link)
        .setURL('https://basementx.lol'),
      new ButtonBuilder()
        .setLabel('🛡️ Bypass Proxy (proxy.basementx.lol)')
        .setStyle(ButtonStyle.Link)
        .setURL('https://proxy.basementx.lol')
    );
  ...
  public static announcement(title: string, message: string, bannerUrl?: string): EmbedBuilder {
    const embed = new EmbedBuilder()
      ...
      .setColor(BASEMENT_COLORS.emerald)
      .setFooter({
        text: 'Basement Announcements • Official Dispatch',
        iconURL: BASEMENT_BRANDING.avatarUrl,
      });
  ...
  public static createHelpMenu(
    ...
    if (page === 0) {
      ...
      title = '🌟 Basement Cinema • Command & Prefix Manual';
      desc =
        `Welcome to the **Basement Cinema Discord Bot**!\n` +
        `You can interact with the bot using **Slash Commands** (\`/command\`) or **Custom Prefix Commands** (\`${p}command\`).\n\n` +
        `**Current Server Prefix:** \`${p}\`\n` +
        `**Official Platform:** [basementx.lol](https://basementx.lol) • High-Speed Ad-Free Streaming\n\n` +
        `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
      ...
      desc =
        `Search movies, TV series, anime, and host watch parties on Basement.\n\n` +
      ...
      desc =
        `Submit content requests, talk to Basement AI, check server status, and official mirrors.\n\n` +
        `### 1. \`${p}request <type> <title> | [notes]\` • \`/request\`\n` +
        `• **Aliases:** \`${p}req\`\n` +
        `• **Description:** Submit a request for missing movies, anime, or series to the Basement team. Opens an interactive staff ticket.\n` +
        ...
        `• **Aliases:** \`${p}ai\`, \`${p}ask\`\n` +
        `• **Description:** Ask Basement Cinema AI for recommendations, trivia, actors, or streaming help. *(You can also directly @mention the bot!)*\n` +
        `• **Prefix Format:** \`${p}chat <message>\`\n` +
        `• **Example:** \`${p}chat Recommend me 3 psychological thrillers like Shutter Island\`\n\n` +
        `### 4. \`${p}status\` • \`/status\`\n` +
        `• **Aliases:** \`${p}ping\`\n` +
        `• **Description:** Checks live health, CDN edge latency, and streaming node status for basementx.lol.\n` +
        `• **Prefix Format:** \`${p}status\`\n\n` +
        `### 5. \`${p}domain\` • \`/domain\`\n` +
        `• **Aliases:** \`${p}site\`, \`${p}url\`, \`${p}link\`, \`${p}mirrors\`\n` +
        `• **Description:** Displays official Basement domains, proxy bypass URLs, and mirror statuses.\n` +
      ...
      desc =
        ...
        `• **Example:** \`${p}say #general Welcome to Basement Cinema!\`\n\n` +
        `### 4. \`${p}announce <#channel> <title> | <message>\` • \`/announce\`\n` +
        `• **Description:** Broadcast a styled Basement Cinema announcement with banner embed.\n` +
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(desc)
      .setFooter({
        text: `Basement Help Manual • Page ${page + 1} of ${totalPages} • Category: ${categoryName}`,
        iconURL: BASEMENT_BRANDING.avatarUrl,
      })
      .setTimestamp();
...
export const ZenoxEmbeds = BasementEmbeds;
