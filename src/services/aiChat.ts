...
export class AiChatService {
  /**
   * Generates a conversational, cinema-savvy reply as Basement AI Assistant
   */
  public static async answer(prompt: string, userTag?: string): Promise<string> {
    ...
    if (/who (are you|made you|created you)|what are you/i.test(p)) {
      return (
        `🎬 **I am Basement** — your official cinema AI assistant and Discord concierge for [**basementx.lol**](https://basementx.lol)!\n\n` +
    ...
    if (/how (to|do i) request|request (movie|show|series)/i.test(p)) {
      return (
        `🍿 **How to Request Titles on Basement:**\n` +
        ...
        `3. Once approved and added, you'll be pinged right here! You can watch immediately on [**basementx.lol**](https://basementx.lol).`
      );
    }

    if (/color role|roles|name color|how (to|do i) get color/i.test(p)) {
      return (
        `🎨 **Custom Name Colors on Basement:**\n` +
    ...
    if (/watch ?party|how to host/i.test(p)) {
      return (
        ...
        `Use \`/watchparty title:[Movie Name] link:[basementx.lol stream link] minutes_from_now:[e.g. 15]\`.\n` +
    ...
    if (/domain|mirror|blocked|isp|url|link|website/i.test(p) && !p.includes('recommend')) {
      return (
        `🌐 **Official Basement Streaming Mirrors:**\n` +
        `• **Official Platform**: [https://basementx.lol](https://basementx.lol)\n` +
        `• **Direct Edge CDN**: [https://stream.basementx.lol](https://stream.basementx.lol)\n` +
        `• **Anti-ISP DNS Gateway**: [https://proxy.basementx.lol](https://proxy.basementx.lol)\n\n` +
    ...
    if (/recommend|what should i watch|give me a (movie|show|series)|suggest/i.test(p)) {
      ...
      if (genre) {
        return (
          `🔥 **Basement Cinema Recommendation (${genre}):**\n\n` +
    ...
    if (/joke|tell me a joke|make me laugh/i.test(p)) {
      const jokes = [
        `Why did the movie star go to the dentist? To get their Hollywood smile polished before premiering on **basementx.lol**! 😂`,
        `Why don't movies like playing hide and seek? Because good plots are always giving themselves away! 🍿`,
        `What is an astronaut's favorite key on the keyboard? The Space bar... perfect for pausing Interstellar on **basementx.lol**! 🚀`,
      ];
      return jokes[Math.floor(Math.random() * jokes.length)];
    }

    // 5. Search check: If user mentions a movie name or asks "is X on basement"
    ...
    if (candidate && candidate.length >= 2 && candidate.length <= 40 && !p.includes('how ') && !p.includes('what ')) {
      try {
        ...
        if (searchResults && searchResults.length > 0) {
          ...
          return (
            `🍿 Found **${item.title}** (${item.year}) on Basement!\n\n` +
            `• **Rating**: ⭐ \`${item.rating}/10\` (${item.votes.toLocaleString()} votes)\n` +
            `• **Genres**: \`${item.genres.join(', ')}\`\n` +
            `• **Overview**: *${item.overview.slice(0, 180)}...*\n\n` +
            `▶ **Instant Stream (4K HDR)**: [Watch on Basement](${item.basementUrl})\n` +
