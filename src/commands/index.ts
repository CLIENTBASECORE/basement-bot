...
import { DEFAULT_SHOW_ROLES, DEFAULT_PING_ROLES, BASEMENT_COLORS, BASEMENT_BRANDING } from '../utils/branding.js';
...
export const searchCommand: Command = {
  data: new SlashCommandBuilder()
    .setName('search')
    .setDescription('Search the Basement TMDB catalog for movies and TV shows')
    .addStringOption(opt =>
  ...
  async execute(interaction) {
    ...
    if (results.length === 0) {
      await interaction.editReply({
        content: `❌ No matching titles found on Basement for \`${query}\`. You can use \`/request\` to ask our team to add it!`,
      });
      return;
    }

    const payload = BasementEmbeds.mediaDetail(results[0]);
...
export const randomCommand: Command = {
  ...
  async execute(interaction) {
    ...
    const payload = BasementEmbeds.mediaDetail(item);
    await interaction.editReply(payload);
  },
};
...
export const requestCommand: Command = {
  data: new SlashCommandBuilder()
    .setName('request')
    .setDescription('Request a missing movie or TV show season to be added to Basement')
    .addStringOption(opt =>
  ...
  async execute(interaction) {
    ...
    if (channelTarget) {
      try {
        const ticketPayload = BasementEmbeds.requestTicket(newRequest);
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `<@${interaction.user.id}> submitted a request to add **${title}** to the Basement catalog!\n\n` +
        `• **Title**: \`${title}\`\n` +
      ...
      .setFooter({ text: 'Basement Media Queue • Streaming on basementx.lol', iconURL: BASEMENT_BRANDING.avatarUrl })
      .setTimestamp();
...
export const watchpartyCommand: Command = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption(opt =>
      opt.setName('link').setDescription('Direct Basement stream URL').setRequired(true)
    )
  ...
  async execute(interaction) {
    ...
    const payload = BasementEmbeds.watchParty(wp);
...
export const nowplayingCommand: Command = {
  data: new SlashCommandBuilder()
    .setName('nowplaying')
    .setDescription('Share what movie or series you are currently watching on Basement')
    .addStringOption(opt =>
  ...
  async execute(interaction) {
    ...
    const embed = BasementEmbeds.base(
      ...
      `<@${interaction.user.id}> is currently streaming **${title}** in Ultra HD!\n\nWant to join in? Search for **${title}** on [Basement](${CONFIG.BASEMENT_BASE_URL}).`
    )
      .setAuthor({
...
export const statusCommand: Command = {
  data: new SlashCommandBuilder()
    .setName('status')
    .setDescription('Check official basementx.lol website status and streaming availability'),
  async execute(interaction) {
    await interaction.deferReply();
    const nodes = await StatusService.refreshStatus();
    const payload = BasementEmbeds.systemStatus(nodes);
    await interaction.editReply(payload);
  },
};
...
export const domainCommand: Command = {
  data: new SlashCommandBuilder()
    .setName('domain')
    .setDescription('Get official working Basement streaming domains and proxy mirrors'),
  async execute(interaction) {
    const payload = BasementEmbeds.domainList();
...
export const chatCommand: Command = {
  data: new SlashCommandBuilder()
    .setName('chat')
    .setDescription('Talk with Basement AI about movies, shows, streaming tips, or recommendations')
    .addStringOption(opt =>
      opt.setName('prompt').setDescription('What would you like to ask Basement?').setRequired(true)
    ),
  async execute(interaction) {
    await interaction.deferReply();
...
export const botinfoCommand: Command = {
  data: new SlashCommandBuilder()
    .setName('botinfo')
    .setDescription('View Basement bot system metrics, uptime, latency, and platform stats'),
  async execute(interaction) {
    ...
    const embed = new EmbedBuilder()
      .setTitle('⚡ Basement Bot Core Telemetry')
      .setColor(BASEMENT_COLORS.emerald)
      .setThumbnail(BASEMENT_BRANDING.avatarUrl)
      .setDescription(
        `Official Discord bot and streaming cinema engine for [**basementx.lol**](https://basementx.lol).\n` +
      ...
      .addFields(
        ...
        { name: '🌐 Official Platform', value: `[basementx.lol](https://basementx.lol)`, inline: true }
      )
      .setFooter({ text: 'Basement Cyberpunk Cinema Edition • v2.4.0-PRO', iconURL: BASEMENT_BRANDING.avatarUrl })
      .setTimestamp();
...
export const sayCommand: Command = {
  data: new SlashCommandBuilder()
    .setName('say')
    .setDescription('Type and send a message or styled embed through Basement to any channel')
    ...
    .addBooleanOption(opt =>
      opt.setName('as_embed').setDescription('Format as a sleek Basement glass embed').setRequired(false)
    )
    .addStringOption(opt =>
  ...
  async execute(interaction) {
    ...
    try {
      ...
      await interaction.reply({
        content: `✅ Message sent through **Basement** to <#${targetChannel.id}>!`,
        flags: MessageFlags.Ephemeral,
      });
    } catch (err: any) {
      await interaction.reply({
...
export const requestsCommand: Command = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'list') {
      ...
      const embed = new EmbedBuilder()
        .setTitle(`🎬 Basement Community Media Requests (${filter.toUpperCase()})`)
        .setColor(BASEMENT_COLORS.emerald)
        .setDescription(
        ...
        .setFooter({ text: 'Basement Cinema Library • basementx.lol', iconURL: BASEMENT_BRANDING.avatarUrl })
        .setTimestamp();
...
export const announceCommand: Command = {
  data: new SlashCommandBuilder()
    .setName('announce')
    .setDescription('Broadcast a styled Basement cinema announcement embed')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .setDMPermission(false)
  ...
  async execute(interaction) {
    ...
    const embed = BasementEmbeds.announcement(title, message, 'https://images.unsplash.com/photo-1518709268805-4e9042af9f23?w=1200');
...
export const panelCommand: Command = {
  data: new SlashCommandBuilder()
    .setName('panel')
    .setDescription('Deploy aesthetic Basement reaction & button role panels')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .setDMPermission(false)
    .addStringOption(opt =>
      opt
  ...
  async execute(interaction) {
    ...
    if (panelType === 'colors' || panelType === 'all') {
      const colorPayload = BasementEmbeds.createColorPanel();
      await targetChannel.send(colorPayload);
    }

    if (panelType === 'pings' || panelType === 'all') {
      const pingPayload = BasementEmbeds.createPingPanel();
      await targetChannel.send(pingPayload);
    }

    if (panelType === 'shows' || panelType === 'all') {
      const showPayload = BasementEmbeds.createShowPanel();
...
export const adminsetupCommand: Command = {
  data: new SlashCommandBuilder()
    .setName('adminsetup')
    .setDescription('Configure Basement administrator channel, synchronize roles, and deploy role panels')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .setDMPermission(false)
    .addSubcommand(sub =>
      sub
    ...
    .addSubcommand(sub =>
      sub
        .setName('status')
        .setDescription('View current Basement admin channel, requests channel, and role synchronization status')
    )
    .addSubcommand(sub =>
      sub
  ...
  async execute(interaction) {
    ...
    if (subcommand === 'channel') {
      ...
      const embed = new EmbedBuilder()
        .setTitle('🛡️ Basement Admin Chat Channel Configured')
        .setColor(BASEMENT_COLORS.emerald)
        .setDescription(
          ...
          `• **Manual File Configuration**: You can also set \`ADMIN_CHANNEL_ID=${channel.id}\` manually in your \`.env\` file or in \`data/basement_db.json\`!\n` +
          `• **Channel Permissions**: Ensure only server administrators have access to <#${channel.id}>.`
        )
        .setFooter({ text: 'Basement Admin Core • basementx.lol', iconURL: BASEMENT_BRANDING.avatarUrl })
        .setTimestamp();
    ...
    if (subcommand === 'requests_channel') {
      ...
      const embed = new EmbedBuilder()
        .setTitle('📥 Basement Media Requests Channel Configured')
        .setColor(BASEMENT_COLORS.emerald)
        .setDescription(
    ...
    if (subcommand === 'color_roles') {
        ...
        if (matchingRole) {
          ...
          try {
            const newRole = await guild.roles.create({
              ...
              permissions: [],
              reason: 'Basement Admin Setup / Color Roles',
            });
            createdCount++;
      ...
      if (deployChannel && deployChannel.type === ChannelType.GuildText) {
        try {
          const panelPayload = BasementEmbeds.createColorPanel(updatedColors);
      ...
      const embed = new EmbedBuilder()
        .setTitle('🎨 Basement Color Roles Setup Complete')
        .setColor(BASEMENT_COLORS.emerald)
        .setDescription(
    ...
    if (subcommand === 'movie_roles') {
      ...
      if (deployChannel && deployChannel.type === ChannelType.GuildText) {
        try {
          const panelPayload = BasementEmbeds.createShowPanel();
    ...
    if (subcommand === 'ping_roles') {
      ...
      if (deployChannel && deployChannel.type === ChannelType.GuildText) {
        try {
          const panelPayload = BasementEmbeds.createPingPanel();
          await deployChannel.send(panelPayload);
          deployMessage = `\n\n🚀 **Live Panel Deployed**: Dispatched to <#${deployChannel.id}>!`;
        } catch (err: any) {
          deployMessage = `\n\n⚠️ Failed to deploy panel to <#${deployChannel.id}>: ${err.message}`;
    ...
    if (subcommand === 'status') {
      ...
      const embed = new EmbedBuilder()
        .setTitle('🛡️ Basement Server Administration Status')
        .setColor(BASEMENT_COLORS.emerald)
        .setDescription(
    ...
    if (subcommand === 'prefix') {
      ...
      const embed = new EmbedBuilder()
        ...
        .setFooter({ text: 'Basement Configuration Manager • basementx.lol', iconURL: BASEMENT_BRANDING.avatarUrl });
...
export const helpCommand: Command = {
  data: new SlashCommandBuilder()
    .setName('help')
    .setDescription('Interactive manual for all Basement bot commands and custom prefix formats')
    .addStringOption(opt =>
      opt
        .setName('category')
  ...
  async execute(interaction) {
    ...
    const payload = BasementEmbeds.createHelpMenu(page, prefix, canAccessAdminHelp);
    await interaction.reply(payload);
  },
};

// Dedicated Administrator & Setup Manual - ADMIN ONLY
export const adminhelpCommand: Command = {
  ...
  async execute(interaction) {
    ...
    const prefix = db.getPrefix();
    const payload = BasementEmbeds.createHelpMenu(3, prefix, true);
