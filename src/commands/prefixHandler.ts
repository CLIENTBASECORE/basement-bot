  ...
  try {
    switch (command) {
      ...
      case 'prefix': {
        if (!args.length) {
          ...
          const embed = new EmbedBuilder()
            .setTitle('⚡ Basement Command Prefix')
            ...
            .setFooter({ text: 'Basement Command Dispatcher • basementx.lol', iconURL: BASEMENT_BRANDING.avatarUrl });
        ...
        const embed = new EmbedBuilder()
          ...
          .setFooter({ text: 'Basement Configuration Manager', iconURL: BASEMENT_BRANDING.avatarUrl });
      ...
      case 'find': {
        ...
        if (results.length === 0) {
          await message.reply(`❌ No matching titles found on Basement for \`${fullArgString}\`. You can request it using \`${prefix}request ${fullArgString}\`!`);
          return;
        }
      ...
      case 'req': {
        ...
        const embed = new EmbedBuilder()
          ...
          .setColor(BASEMENT_COLORS.emerald)
          .setDescription(
            `<@${message.author.id}> requested **${title}** to be added to the Basement catalog!\n\n` +
            `• **Title**: \`${title}\`\n` +
      ...
      case 'reqs': {
        ...
        if (sub === 'resolve') {
          ...
          if (updated.channelId && updated.messageId && message.guild) {
            try {
              ...
              if (originalMsg) {
                await originalMsg.edit(BasementEmbeds.requestTicket(updated) as any);
              }
            } catch {}
          }
          ...
          try {
            ...
            if (targetUser) {
              const dmEmbed = BasementEmbeds.base(
                ...
                (newStatus === 'added' ? `▶ It is now live on [Basement](https://basementx.lol)!` : `Thank you for contributing to the Basement catalog.`)
              );
        ...
        const embed = new EmbedBuilder()
          ...
          .setFooter({ text: 'Basement Content Queue • basementx.lol', iconURL: BASEMENT_BRANDING.avatarUrl });
      ...
      case 'wp': {
        ...
        const link = parts[2] || 'https://basementx.lol';
      ...
      case 'about': {
        ...
        const embed = new EmbedBuilder()
          .setTitle('🤖 Basement Cinema Bot Specifications')
          .setColor(BASEMENT_COLORS.emerald)
          .setThumbnail(BASEMENT_BRANDING.avatarUrl)
          .setDescription(
            `**Basement Cinema Network • Pure Discord Edition**\n` +
            ...
            `• **Support Server:** [Join Basement Discord](${BASEMENT_BRANDING.supportDiscord})`
          )
          .setFooter({ text: 'Basement System Architecture • basementx.lol', iconURL: BASEMENT_BRANDING.avatarUrl })
          .setTimestamp();
      ...
      case 'say': {
        ...
        if (isEmbed) {
          ...
          const embed = new EmbedBuilder()
            ...
            .setDescription(desc)
            .setColor(BASEMENT_COLORS.emerald)
            .setFooter({ text: 'Basement Cinema Broadcast', iconURL: BASEMENT_BRANDING.avatarUrl })
            .setTimestamp();
      ...
      case 'announce': {
        ...
        if (!match || !match[2].includes('|')) {
          await message.reply(
            ...
            `*Example:* \`${prefix}announce #announcements New 4K Releases | Added 15 new movies to basementx.lol!\``
          );
          return;
        }
        ...
        const embed = BasementEmbeds.announcement(title, body, 'https://images.unsplash.com/photo-1518709268805-4e9042af9f23?w=1200');
        await targetCh.send({ embeds: [embed] });
    ...
    console.error(`[Basement Prefix Command Error: ${command}]`, err);
