...
const DB_PATH = path.join(DATA_DIR, 'basement_db.json');

const INITIAL_NODES: NodeStatus[] = [
  {
    id: 'basement-web-1',
    name: 'Basement Official Web Streaming Platform',
    endpoint: 'https://basementx.lol',
    status: 'online',
    latencyMs: 22,
    uptime: 99.99,
    type: 'web',
  ...
  },
  {
    ...
    endpoint: 'https://proxy.basementx.lol',
    status: 'online',
    latencyMs: 48,
    uptime: 99.90,
    type: 'proxy',
  ...
  constructor() {
    ...
    if (fs.existsSync(DB_PATH)) {
      try {
        ...
        console.error('Failed to parse existing basement_db.json, initializing fresh DB.', e);
